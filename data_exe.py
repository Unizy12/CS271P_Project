    ...
    for pointlists in polygons:
        ...
        for pointlist in pointlists:
            ...
            if min_x > 0 and min_y > 0 and max_x > 0 and max_y > 0:
                # imgname, img_width, img_height, min_x, min_y, max_x, max_y
                data_row = [img_name,np.size(img, 1),np.size(img, 0), min_x, min_y, max_x, max_y]
...
def generate_label_files(image_dir):
    header = ['filename','class', 'width', 'height',
               'xmin', 'ymin', 'xmax', 'ymax']
    for root, dirs, filenames in os.walk(image_dir):
        for dir in dirs:
            csvholder = []
            ...
            for f in os.listdir(image_dir + dir):
                if(f.split(".")[1] == "csv"):
                    ...
                    for row in reader:
                        [imgname, img_width, img_height, min_x, min_y, max_x, max_y] = row
                        csvholder.append([imgname,'hand', img_width, img_height, min_x, min_y, max_x, max_y])
                    csv_file.close()
...
def merge(path):
    for root, dirs, filenames in os.walk(path):
        for filename in filenames:
            os.rename(path+'/'+filename,'/home/unizy/DeepLearningExamples/PyTorch/Detection/SSD/coco/test'+'/'+filename)
if __name__ == '__main__':
    ...
    # merge('/home/unizy/DeepLearningExamples/PyTorch/Detection/SSD/images/test')
    from skimage import io
    path = './Hand_data/out/1'
